...
class NotificationSettingsFragment : PreferenceFragmentCompat() {
    override fun onCreatePreferences(savedInstanceState: Bundle, rootKey: String) {
        ...
        (requireActivity() as SettingsActivity).loadedFragments++
        setNotif()
        var myPref = findPreference<Preference>("timetableNotif")
        ...
        Objects.requireNonNull(reminderSwitch).summary = getString(
            string.notification_reminder_settings_desc,
            "" + PreferenceUtil.getReminderTime(requireContext())
        )
        Objects.requireNonNull(reminderSwitch).onPreferenceClickListener =
            Preference.OnPreferenceClickListener { p: Preference? ->
                if (reminderSwitch!!.isChecked) {
                    val numberPicker = NumberPicker(context)
                    val max = PreferenceUtil.getPeriodLength(requireContext()) - 2
                    numberPicker.maxValue = if (max <= 0) 2 else max
                    numberPicker.minValue = 1
                    numberPicker.value = PreferenceUtil.getReminderTime(requireContext())
                    MaterialDialog.Builder(requireContext())
                        ...
                        .onPositive { d: MaterialDialog?, w: DialogAction? ->
                            ...
                            reminderSwitch.summary =
                                getString(string.notification_reminder_settings_desc, "" + value)
                        }
                        .show()
                }
                true
        ...
        Objects.requireNonNull(myPref).onPreferenceClickListener =
            Preference.OnPreferenceClickListener { p: Preference ->
                val oldTimes = PreferenceUtil.getAlarmTime(
                    context
                )
                val timePickerDialog = TimePickerDialog(activity,
                    { view: TimePicker?, hourOfDay: Int, minute: Int ->
                        ...
                        PreferenceUtil.setRepeatingAlarm(
                            ...
                            minute,
                            0,
                            DailyReceiver.Companion.DailyReceiverID,
        ...
        val oldTimes = PreferenceUtil.getAlarmTime(context)
        myPref!!.summary = oldTimes[0].toString() + ":" + oldTimes[1]
    }

    private fun setNotif() {
        val show = PreferenceManager.getDefaultSharedPreferences(requireContext())
            .getBoolean("timetableNotif", true)
        findPreference<Preference>("alwaysNotification")!!.isVisible = show
        findPreference<Preference>("alarm")!!.isVisible = show
        findPreference<Preference>("reminder")!!.isVisible =
            show && ProfileManagement.isPreferredProfile()
        findPreference<Preference>("notification_end")!!.isVisible =
            show && ProfileManagement.isPreferredProfile()
    }
}